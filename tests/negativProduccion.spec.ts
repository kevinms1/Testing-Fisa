...
test('-+Comunidad: siguiente', async ({ page }) => {
 ...
 test('-+PM siguiente', async ({ page }) => {
  ...
  //       test('PM siguiente 3', async ({ page }) => {
  // const nextCaseNumber = fs.readFileSync('caso.txt', 'utf8').trim();
  // console.log('Número de caso leído:', nextCaseNumber);
  // test.setTimeout(100000);
  ...
  // await expect(page.locator('#tab-5')).toContainText('Certificado');
  // await page.getByRole('button', { name: 'Modificar Puesta en Producción' }).click();
  // await page.getByRole('combobox', { name: 'Puesta en Producción' }).click();        
  //  });

   
    test('-+Comunidad: siguiente certificado', async ({ page }) => {
  const caseNumber = fs.readFileSync('caso.txt', 'utf8').trim();
  const nextCaseNumber = String(Number(caseNumber) + 1).padStart(caseNumber.length, '0');
  console.log('Número de caso leído:', caseNumber);
  console.log('Número de caso Hijo:', nextCaseNumber);
  ...
  await expect(page.locator('#tab-5')).toContainText('En Producción');          
      ...
      test('-+Comunidad: produccion', async ({ page }) => {
  const caseNumber = fs.readFileSync('caso.txt', 'utf8').trim();
  const nextCaseNumber = String(Number(caseNumber) + 1).padStart(caseNumber.length, '0');
  console.log('Número de caso leído:', caseNumber);
  console.log('Número de caso Hijo:', nextCaseNumber);
  ...
  await page.getByText('Entrega incompleta').click();
  await page.getByLabel('*Comentario').fill('datos de prueba');
  await page.getByText('Guardar').click();
});
    ...
    expect (page.getByText('Estado: En Revisión Producción', { exact: true }));
